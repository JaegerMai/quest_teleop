...
class MetaQuestController:
    ...
    def input2action(self, controller_type="POSE"):
        """Convert input from the VR controller to action space.
           params:
               controller_type (str): The type of controller input ("POSE": position and orientation, "YAW": yaw rotation + position, "POSITION": position only).
        """
        ...
        if controller_type == "POSE":
            drot = d_rotation_mat.as_rotvec()
        if controller_type == "YAW":
